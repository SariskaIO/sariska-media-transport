...
export default class JitsiLocalTrack extends JitsiTrack {
  ...
  constructor({
    ...
    () => this.emit(LOCAL_TRACK_STOPPED), mediaType, videoType);
    ...
    const displaySurface = videoType === VideoType.DESKTOP ? track.getSettings().displaySurface : null;
    /**
     * Track metadata.
     */
    ...
    this.sourceType = sourceType; // Get the resolution from the track itself because it cannot be
    // certain which resolution webrtc has fallen back to using.

    this.resolution = track.getSettings().height;
    this.maxEnabledResolution = resolution; // Cache the constraints of the track in case of any this track
    ...
    this._realDeviceId = this.deviceId === '' ? undefined : this.deviceId; // The source name that will be signaled for this track.
  ...
  _fireNoDataFromSourceEvent() {
    ...
    this.emit(NO_DATA_FROM_SOURCE, value); // FIXME: Should we report all of those events
  ...
  _setMuted(muted) {
    ...
    if (this.isAudioTrack() || this.videoType === VideoType.DESKTOP || !browser.doesVideoMuteByStreamRemove()) {
      logMuteInfo(); // If we have a stream effect that implements its own mute functionality, prioritize it before
      ...
      promise = RTCUtils.obtainAudioAndVideoPermissions(Object.assign({}, streamOptions, {
        constraints: {
  ...
  _switchCamera() {
    if (this.isVideoTrack() && this.videoType === VideoType.CAMERA && typeof this.track._switchCamera === 'function') {
      this.track._switchCamera();

      this._facingMode = this._facingMode === CameraFacingMode.ENVIRONMENT ? CameraFacingMode.USER : CameraFacingMode.ENVIRONMENT;
    }
  }
  /**
  ...
  dispose() {
    let promise = Promise.resolve(); // Remove the effect instead of stopping it so that the original stream is restored
  ...
  getCameraFacingMode() {
    if (this.isVideoTrack() && this.videoType === VideoType.CAMERA) {
      var _this$track$getSettin, _this$track;
      ...
      const trackSettings = (_this$track$getSettin = (_this$track = this.track).getSettings) === null || _this$track$getSettin === void 0 ? void 0 : _this$track$getSettin.call(_this$track);
  ...
  isReceivingData() {
    if (this.isVideoTrack() && (this.isMuted() || this._stopStreamInProgress || this.videoType === VideoType.DESKTOP)) {
      return true;
    }
      ...
      setTimeout(() => {
        if (!this._hasSentData) {
          logger.warn(`${this} 'bytes sent' <= 0: \
                        ${bytesSent}`);
          Statistics.analytics.sendEvent(NO_BYTES_SENT, {
            'media_type': this.getType()
          });
        }
      }, 3000);
