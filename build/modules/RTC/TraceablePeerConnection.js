...
import { HD_BITRATE, HD_SCALE_FACTOR, SIM_LAYER_RIDS, TPCUtils } from './TPCUtils'; // FIXME SDP tools should end up in some kind of util module
...
export default function TraceablePeerConnection(rtc, id, signalingLayer, pcConfig, constraints, isP2P, options) {
  ...
  this.options = options; // Make sure constraints is properly formatted in order to provide information about whether or not this
  // connection is P2P to rtcstats.

  const safeConstraints = constraints || {};
  safeConstraints.optional = safeConstraints.optional || []; // The `optional` parameter needs to be of type array, otherwise chrome will throw an error.
  // Firefox and Safari just ignore it.

  if (Array.isArray(safeConstraints.optional)) {
    safeConstraints.optional.push({
  ...
  */
  ...
  this._senderVideoMaxHeight = 2160; // override as desired
...
TraceablePeerConnection.prototype.getDesiredMediaDirection = function (mediaType, isAddOperation = false) {
  ...
  if (this._usesUnifiedPlan) {
    return isAddOperation ? hasLocalSource ? MediaDirection.SENDRECV : MediaDirection.SENDONLY : hasLocalSource ? MediaDirection.RECVONLY : MediaDirection.INACTIVE;
  }
...
TraceablePeerConnection.prototype._getReceiversByEndpointIds = function (endpoints, mediaType) {
  ...
  const remoteTrackIds = remoteTracks.map(remote => {
    ...
    return (_remote$track = remote.track) === null || _remote$track === void 0 ? void 0 : _remote$track.id;
  });
...
TraceablePeerConnection.prototype.getAudioLevels = function (speakerList = []) {
  const audioLevels = {};
  const audioReceivers = speakerList.length ? this._getReceiversByEndpointIds(speakerList, MediaType.AUDIO) : this.peerconnection.getReceivers().filter(receiver => receiver.track && receiver.track.kind === MediaType.AUDIO && receiver.track.enabled);
...
TraceablePeerConnection.prototype._remoteTrackAdded = function (stream, track, transceiver = null) {
  ...
  logger.info(`${this} adding remote track for stream[id=${streamId},type=${mediaType}]`); // look up an associated JID for a stream id

  if (!mediaType) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`MediaType undefined for remote track, stream id: ${streamId}`)); // Abort

    return;
  }

  const remoteSDP = this._usesUnifiedPlan ? new SDP(this.peerconnection.remoteDescription.sdp) : new SDP(this.remoteDescription.sdp);
  let mediaLines; // In unified plan mode, find the matching mline using 'mid' if its availble, otherwise use the
  ...
  if (!mediaLines.length) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No media lines found in remote SDP for remote stream[id=${streamId},type=${mediaType}]`)); // Abort
  ...
  if (!ssrcLines.length) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No SSRC lines found in remote SDP for remote stream[msid=${streamId},type=${mediaType}]`)); // Abort
  ...
  if (isNaN(trackSsrc) || trackSsrc < 0) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`Invalid SSRC for remote stream[ssrc=${trackSsrc},id=${streamId},type=${mediaType}]`)); // Abort

    return;
  } else if (!ownerEndpointId) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No SSRC owner known for remote stream[ssrc=${trackSsrc},id=${streamId},type=${mediaType}]`)); // Abort
...
TraceablePeerConnection.prototype._createRemoteTrack = function (ownerEndpointId, stream, track, mediaType, videoType, ssrc, muted, sourceName) {
  ...
  if ((userTracksByMediaType === null || userTracksByMediaType === void 0 ? void 0 : userTracksByMediaType.size) && Array.from(userTracksByMediaType).find(jitsiTrack => jitsiTrack.getTrack() === track)) {
    ...
    logger.error(`${this} received a second remote track for track[endpoint=${ownerEndpointId},type=${mediaType}]` + 'deleting the existing track');
    const existingTrack = Array.from(userTracksByMediaType)[0]; // The exisiting track needs to be removed here. This happens on Safari sometimes when a SSRC is removed from
...
TraceablePeerConnection.prototype._removeRemoteTrack = function (toBeRemoved) {
  ...
  } else if (!((_userTracksByMediaTyp = userTracksByMediaType.get(toBeRemoved.getType())) === null || _userTracksByMediaTyp === void 0 ? void 0 : _userTracksByMediaTyp.delete(toBeRemoved))) {
...
TraceablePeerConnection.prototype._extractSSRCMap = function (desc) {
  ...
  let media = session.media; // For unified plan clients, only the first audio and video mlines will have ssrcs for the local sources.
  ...
  for (const mLine of media) {
    ...
    let ssrcs = mLine.ssrcs; // Filter the ssrcs with 'msid' attribute for plan-b clients and 'cname' for unified-plan clients.
...
function replaceDefaultUnifiedPlanMsid(ssrcLines = []) {
  ...
  problematicSsrcIds.forEach(ssrcId => {
    ...
    cnameLine.value = `${MediaDirection.RECVONLY}-${ssrcId}`; // Remove all of lines for the ssrc.
...
const enforceSendRecv = function (localDescription, options) {
  var _transformer$selectMe, _transformer$selectMe2;
  ...
  const audioMedia = (_transformer$selectMe = transformer.selectMedia(MediaType.AUDIO)) === null || _transformer$selectMe === void 0 ? void 0 : _transformer$selectMe[0];
  let changed = false;
  ...
  const videoMedia = (_transformer$selectMe2 = transformer.selectMedia(MediaType.VIDEO)) === null || _transformer$selectMe2 === void 0 ? void 0 : _transformer$selectMe2[0];
...
const getters = {
  ...
  localDescription() {
    ...
    this.trace('getLocalDescription::preTransform', dumpSDP(desc)); // If the browser is running in unified plan mode and this is a jvb connection,
...
TraceablePeerConnection.prototype.addTrack = function (track, isInitiator = false) {
  ...
  if (this._usesUnifiedPlan) {
    ...
    } else if (!browser.doesVideoMuteByStreamRemove() || track.isAudioTrack() || track.isVideoTrack() && !track.isMuted()) {
...
TraceablePeerConnection.prototype.getConfiguredVideoCodec = function () {
  ...
  const sdp = (_this$peerconnection$ = this.peerconnection.localDescription) === null || _this$peerconnection$ === void 0 ? void 0 : _this$peerconnection$.sdp;
...
TraceablePeerConnection.prototype.replaceTrack = function (oldTrack, newTrack) {
  ...
  if (this._usesUnifiedPlan) {
    ...
    return promise.then(transceiver => {
      oldTrack && this.localTracks.delete(oldTrack.rtcId);
      newTrack && this.localTracks.set(newTrack.rtcId, newTrack);
      const mediaActive = mediaType === MediaType.AUDIO ? this.audioTransferActive : this.videoTransferActive; // Set the transceiver direction only if media is not suspended on the connection. This happens when
...
TraceablePeerConnection.prototype._adjustLocalMediaDirection = function (localDescription) {
  var _transformer$selectMe3, _transformer$selectMe4;

  const transformer = new SdpTransformWrap(localDescription.sdp);
  let modifiedDirection = false;
  const audioMedia = (_transformer$selectMe3 = transformer.selectMedia(MediaType.AUDIO)) === null || _transformer$selectMe3 === void 0 ? void 0 : _transformer$selectMe3[0];
  ...
  const videoMedia = (_transformer$selectMe4 = transformer.selectMedia(MediaType.VIDEO)) === null || _transformer$selectMe4 === void 0 ? void 0 : _transformer$selectMe4[0];
...
TraceablePeerConnection.prototype._adjustRemoteMediaDirection = function (remoteDescription) {
  ...
  [MediaType.AUDIO, MediaType.VIDEO].forEach(mediaType => {
    var _transformer$selectMe5;

    const media = (_transformer$selectMe5 = transformer.selectMedia(mediaType)) === null || _transformer$selectMe5 === void 0 ? void 0 : _transformer$selectMe5[0];
    const hasLocalSource = this.hasAnyTracksOfType(mediaType);
...
TraceablePeerConnection.prototype._mungeOpus = function (description) {
  ...
  for (const mLine of mLines) {
    if (mLine.type === 'audio') {
      const {
        payload
      } = mLine.rtp.find(protocol => protocol.codec === CodecMimeType.OPUS);
...
TraceablePeerConnection.prototype.setLocalDescription = function (description) {
  ...
  this.trace('setLocalDescription::preTransform', dumpSDP(localDescription)); // Munge stereo flag and opusMaxAverageBitrate based on config.js
...
TraceablePeerConnection.prototype.setAudioTransferActive = function (active) {
  ...
  if (this._usesUnifiedPlan) {
    this.tpcUtils.setAudioTransferActive(active); // false means no renegotiation up the chain which is not needed in the Unified mode
...
TraceablePeerConnection.prototype.setRemoteDescription = function (description) {
  ...
  this.trace('setRemoteDescription::preTransform', dumpSDP(description)); // Munge stereo flag and opusMaxAverageBitrate based on config.js
...
TraceablePeerConnection.prototype.setSenderVideoConstraints = function (frameHeight) {
  ...
  if (!(parameters === null || parameters === void 0 ? void 0 : (_parameters$encodings = parameters.encodings) === null || _parameters$encodings === void 0 ? void 0 : _parameters$encodings.length)) {
    return Promise.resolve();
  } // Set the degradation preference.


  const preference = this.isSharingLowFpsScreen() ? DEGRADATION_PREFERENCE_DESKTOP // Prefer resolution for low fps share.
  : DEGRADATION_PREFERENCE_CAMERA; // Prefer frame-rate for high fps share and camera.

  parameters.degradationPreference = preference;
  logger.info(`${this} Setting degradation preference [preference=${preference},track=${localVideoTrack}`); // Calculate the encodings active state based on the resolution requested by the bridge.
  ...
  if (this.isSimulcastOn()) {
    for (const encoding in parameters.encodings) {
      if (parameters.encodings.hasOwnProperty(encoding)) {
        var _this$options, _this$options$videoQu;

        parameters.encodings[encoding].active = this.encodingsEnabledState[encoding]; // Firefox doesn't follow the spec and lets application specify the degradation preference on the
        // encodings.

        browser.isFirefox() && (parameters.encodings[encoding].degradationPreference = preference); // Max bitrates are configured on the encodings only for VP8.

        if (this.getConfiguredVideoCodec() === CodecMimeType.VP8 && (((_this$options = this.options) === null || _this$options === void 0 ? void 0 : (_this$options$videoQu = _this$options.videoQuality) === null || _this$options$videoQu === void 0 ? void 0 : _this$options$videoQu.maxBitratesVideo) || this.isSharingLowFpsScreen() || this._usesUnifiedPlan)) {
          parameters.encodings[encoding].maxBitrate = maxBitrates[encoding];
        }
      }
    }

    this.tpcUtils.updateEncodingsResolution(parameters); // For p2p and cases and where simulcast is explicitly disabled.
    ...
    browser.isFirefox() && (parameters.encodings[0].degradationPreference = preference); // Configure the bitrate.

    if (this.getConfiguredVideoCodec() === CodecMimeType.VP8 && ((_this$options2 = this.options) === null || _this$options2 === void 0 ? void 0 : (_this$options2$videoQ = _this$options2.videoQuality) === null || _this$options2$videoQ === void 0 ? void 0 : _this$options2$videoQ.maxBitratesVideo)) {
      var _this$getTargetVideoB;

      let bitrate = (_this$getTargetVideoB = this.getTargetVideoBitrates()) === null || _this$getTargetVideoB === void 0 ? void 0 : _this$getTargetVideoB.high;

      if (videoType === VideoType.CAMERA) {
        var _this$tpcUtils$localS, _this$tpcUtils$localS2;

        bitrate = (_this$tpcUtils$localS = (_this$tpcUtils$localS2 = this.tpcUtils.localStreamEncodingsConfig.find(layer => layer.scaleResolutionDownBy === scaleFactor)) === null || _this$tpcUtils$localS2 === void 0 ? void 0 : _this$tpcUtils$localS2.maxBitrate) !== null && _this$tpcUtils$localS !== void 0 ? _this$tpcUtils$localS : bitrate;
      }
...
TraceablePeerConnection.prototype.setVideoTransferActive = function (active) {
  ...
  if (this._usesUnifiedPlan) {
    this.tpcUtils.setVideoTransferActive(active); // false means no renegotiation up the chain which is not needed in the Unified mode
...
TraceablePeerConnection.prototype._createOfferOrAnswer = function (isOffer, constraints) {
  ...
  const handleSuccess = (resultSdp, resolveFn, rejectFn) => {
    try {
      ...
      if (!this._usesUnifiedPlan) {
        ...
        if (!this.hasAnyTracksOfType(MediaType.VIDEO) && !this.sdpConsistency.hasPrimarySsrcCached()) {
      ...
      const localVideoTrack = this.getLocalVideoTracks()[0]; // Configure simulcast for camera tracks and for desktop tracks that need simulcast.

      if (this.isSimulcastOn() && browser.usesSdpMungingForSimulcast() && ((localVideoTrack === null || localVideoTrack === void 0 ? void 0 : localVideoTrack.getVideoType()) === VideoType.CAMERA || this._usesUnifiedPlan || !this.isSharingLowFpsScreen())) {
        // eslint-disable-next-line no-param-reassign
  ...
  if (this._usesTransceiverCodecPreferences) {
    const transceiver = this.peerconnection.getTransceivers().find(t => {
      var _t$receiver, _t$receiver$track;

      return t.receiver && ((_t$receiver = t.receiver) === null || _t$receiver === void 0 ? void 0 : (_t$receiver$track = _t$receiver.track) === null || _t$receiver$track === void 0 ? void 0 : _t$receiver$track.kind) === MediaType.VIDEO;
    });

    if (transceiver) {
      var _RTCRtpReceiver$getCa, _this$codecPreference, _this$codecPreference2;

      let capabilities = (_RTCRtpReceiver$getCa = RTCRtpReceiver.getCapabilities(MediaType.VIDEO)) === null || _RTCRtpReceiver$getCa === void 0 ? void 0 : _RTCRtpReceiver$getCa.codecs;
      const mimeType = (_this$codecPreference = this.codecPreference) === null || _this$codecPreference === void 0 ? void 0 : _this$codecPreference.mimeType;
      const enable = (_this$codecPreference2 = this.codecPreference) === null || _this$codecPreference2 === void 0 ? void 0 : _this$codecPreference2.enable;
      ...
      if (capabilities && mimeType && enable) {
        ...
        capabilities = capabilities.filter(caps => caps.mimeType.toLowerCase() !== `${MediaType.VIDEO}/${mimeType}`);
      } // Disable ulpfec on Google Chrome and derivatives because
      // https://bugs.chromium.org/p/chromium/issues/detail?id=1276427


      if (browser.isChromiumBased()) {
        capabilities = capabilities.filter(caps => caps.mimeType.toLowerCase() !== `${MediaType.VIDEO}/${CodecMimeType.ULPFEC}`);
      }
...
TraceablePeerConnection.prototype.getActiveSimulcastStreams = function () {
  ...
  if (this.isSimulcastOn() && this.encodingsEnabledState) {
    var _this$encodingsEnable;

    activeStreams = (_this$encodingsEnable = this.encodingsEnabledState.filter(stream => Boolean(stream))) === null || _this$encodingsEnable === void 0 ? void 0 : _this$encodingsEnable.length;
...
TraceablePeerConnection.prototype.generateNewStreamSSRCInfo = function (track) {
  ...
  if (this.isSimulcastOn() && (track.getVideoType() === VideoType.CAMERA || !this.isSharingLowFpsScreen())) {
    ssrcInfo = {
      ssrcs: [],
      groups: []
    };
