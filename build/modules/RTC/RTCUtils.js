...
const logger = getLogger(__filename); // Require adapter only for certain browsers. This is being done for
...
let disableAGC = false; // Enables stereo.
    ...
    if (browser.isWebKitBased()) {
      if (constraints.video.height && constraints.video.height.ideal) {
        constraints.video.height = {
          ideal: constraints.video.height.ideal
        };
      } else {
      ...
      if (constraints.video.width && constraints.video.width.ideal) {
        constraints.video.width = {
          ideal: constraints.video.width.ideal
        };
      } else {
...
class RTCUtils extends Listenable {
  ...
  init(options = {}) {
    ...
    if (typeof ((_options$audioQuality = options.audioQuality) === null || _options$audioQuality === void 0 ? void 0 : _options$audioQuality.stereo) === 'boolean') {
    ...
    if (browser.isReactNative()) {
      ...
      this.getStreamID = function ({
        ...
        // characters.
        return typeof id === 'number' ? id : SDPUtil.filterSpecialChars(id);
      };
  ...
  obtainAudioAndVideoPermissions(options) {
    ...
    const maybeRequestDesktopDevice = function () {
      ...
      } = otherOptions; // Attempt to use a video input device as a screenshare source if
      ...
      if (desktopSharingSourceDevice) {
        ...
        const constraints = {
          video: {
            deviceId: matchingDevice.deviceId // frameRate is omited here on purpose since this is a device that we'll pretend is a screen.
