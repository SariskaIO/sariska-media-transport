...
export class TPCUtils {
  ...
  findTransceiver(mediaType, localTrack = null) {
    const transceiver = (localTrack === null || localTrack === void 0 ? void 0 : localTrack.track) && localTrack.getOriginalStream() ? this.pc.peerconnection.getTransceivers().find(t => {
      var _t$sender, _t$sender$track;

      return ((_t$sender = t.sender) === null || _t$sender === void 0 ? void 0 : (_t$sender$track = _t$sender.track) === null || _t$sender$track === void 0 ? void 0 : _t$sender$track.id) === localTrack.getTrackId();
    }) : this.pc.peerconnection.getTransceivers().find(t => {
      var _t$receiver, _t$receiver$track;

      return ((_t$receiver = t.receiver) === null || _t$receiver === void 0 ? void 0 : (_t$receiver$track = _t$receiver.track) === null || _t$receiver$track === void 0 ? void 0 : _t$receiver$track.kind) === mediaType;
    });
    return transceiver;
  }
  /**
  ...
  insertUnifiedPlanSimulcastReceive(desc) {
    ...
    const simulcastLine = browser.isFirefox() && browser.isVersionGreaterThan(71) ? `recv ${SIM_LAYER_RIDS.join(';')}` : `recv rid=${SIM_LAYER_RIDS.join(';')}`; // eslint-disable-next-line camelcase
  ...
  */
  ...
  replaceTrack(oldTrack, newTrack) {
    var _newTrack$getType, _newTrack$getTrack, _this$pc$getLocalTrac;
    ...
    const track = (_newTrack$getTrack = newTrack === null || newTrack === void 0 ? void 0 : newTrack.getTrack()) !== null && _newTrack$getTrack !== void 0 ? _newTrack$getTrack : null;
    let transceiver; // If old track exists, replace the track on the corresponding sender.

    if (oldTrack) {
      transceiver = this.pc.peerconnection.getTransceivers().find(t => t.sender.track === oldTrack.getTrack()); // Find the first recvonly transceiver when more than one track of the same media type is being added to the pc.
      // As part of the track addition, a new m-line was added to the remote description with direction set to
      // recvonly.
    } else if (((_this$pc$getLocalTrac = this.pc.getLocalTracks(mediaType)) === null || _this$pc$getLocalTrac === void 0 ? void 0 : _this$pc$getLocalTrac.length) && !newTrack.conference) {
      transceiver = this.pc.peerconnection.getTransceivers().find(t => t.receiver.track.kind === mediaType && t.direction === MediaDirection.RECVONLY && t.currentDirection === MediaDirection.INACTIVE); // For unmute operations, find the transceiver based on the track index in the source name if present, otherwise
      ...
      if (sourceName) {
        ...
        if (trackIndex) {
          transceiver = this.pc.peerconnection.getTransceivers().filter(t => t.receiver.track.kind === mediaType && t.direction !== MediaDirection.RECVONLY)[trackIndex];
        }
      }
    }
  ...
  */
  ...
  setEncodings(track) {
    var _transceiver$sender, _parameters$encodings;

    const mediaType = track.getType();
    const transceiver = this.findTransceiver(mediaType, track);
    const parameters = transceiver === null || transceiver === void 0 ? void 0 : (_transceiver$sender = transceiver.sender) === null || _transceiver$sender === void 0 ? void 0 : _transceiver$sender.getParameters(); // Resolve if the encodings are not available yet. This happens immediately after the track is added to the
  ...
  */
  ...
  updateEncodingsResolution(parameters) {
    ...
    const allEqualEncodings = encodings => encodings.every(encoding => typeof encoding.scaleResolutionDownBy !== 'undefined' && encoding.scaleResolutionDownBy === encodings[0].scaleResolutionDownBy); // Implement the workaround only when all the encodings report the same resolution.
