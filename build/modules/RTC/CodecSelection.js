...
export class CodecSelection {
  ...
  constructor(conference, options) {
    ...
    this.options = options; // VP8 cannot be disabled and it will be the default codec when no preference is set.

    this.disabledCodec = options.disabledCodec === CodecMimeType.VP8 ? undefined : this._getCodecMimeType(options.disabledCodec); // Check if the codec values passed are valid.
    ...
    logger.debug(`Codec preferences for the conference are JVB: ${this.jvbPreferredCodec},
            P2P: ${this.p2pPreferredCodec}`); // Do not prefer VP9 on Firefox because of the following bug.
    ...
    this.conference.on(JitsiConferenceEvents._MEDIA_SESSION_STARTED, session => this._onMediaSessionStared(session));
  }
  /**
  ...
  _isCodecSupported(preferredCodec) {
    ...
    return window.RTCRtpReceiver && window.RTCRtpReceiver.getCapabilities && window.RTCRtpReceiver.getCapabilities('video').codecs.some(codec => codec.mimeType.toLowerCase() === `video/${preferredCodec}`);
  }
  /**
  ...
  _onMediaSessionStared(mediaSession) {
    ...
    const disabledCodec = this.disabledCodec && this._isCodecSupported(this.disabledCodec) ? this.disabledCodec : null;
  ...
  _selectPreferredCodec(mediaSession = null, preferredCodec = null, disabledCodec = null) {
    ...
    if (session && !session.isP2P && !this.options.enforcePreferredCodec) {
      ...
      for (const remote of remoteParticipants) {
        ...
        const peerCodec = peerMediaInfo === null || peerMediaInfo === void 0 ? void 0 : peerMediaInfo.codecType; // We do not want Firefox to switch to VP9 because of the following bug.
        // https://bugzilla.mozilla.org/show_bug.cgi?id=1492500.

        if (peerCodec && peerCodec !== currentCodec && !(browser.isFirefox() && peerCodec === CodecMimeType.VP9)) {
