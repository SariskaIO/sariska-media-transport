...
export const MAIN_THREAD_NEW_DATA_ARRIVED = 'MAIN_THREAD_NEW_DATA_ARRIVED'; // Messages sent by the WebWorker
...
export const WORKER_BLOB_READY = 'WORKER_BLOB_READY'; // Messages sent by either the main thread or the WebWorker
...
class Encoder {
  ...
  constructor(sampleRate, bitDepth = 16, bufferSize = 4096) {
    _defineProperty(this, "_encoderId", 0);

    _defineProperty(this, "_sampleRate", void 0);

    _defineProperty(this, "_bitDepth", void 0);

    _defineProperty(this, "_bufferSize", void 0);

    _defineProperty(this, "_flacBuffers", []);

    _defineProperty(this, "_flacLength", 0);

    _defineProperty(this, "_state", EncoderState.UNINTIALIZED);

    _defineProperty(this, "_data", null);

    _defineProperty(this, "_onMetadataAvailable", () => {// reserved for future use
    });
    ...
    this._bufferSize = bufferSize; // create the encoder

    this._encoderId = Flac.init_libflac_encoder(this._sampleRate, // Mono channel
    1, this._bitDepth, FLAC_COMPRESSION_LEVEL, // Pass 0 in because of unknown total samples,
    0, // checksum, FIXME: double-check whether this is necessary
    true, // Auto-determine block size (samples per frame)
    0);
  ...
  encode(audioData) {
    ...
    const bufferLength = audioData.length; // Convert sample to signed 32-bit integers.
  ...
  finish() {
    if (this._state === EncoderState.WORKING) {
      ...
      console.log('Flac encoding finished: ', status); // free up resources
