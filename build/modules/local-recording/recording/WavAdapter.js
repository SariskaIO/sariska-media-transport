...
export class WavAdapter extends AbstractAudioContextAdapter {
  ...
  constructor() {
    ...
    _defineProperty(this, "_wavLength", 0);

    _defineProperty(this, "_wavBuffers", []);
  ...
  _createWavHeader(dataLength) {
    ...
    const view = new DataView(buffer); // RIFF chunk descriptor

    writeUTFBytes(view, 0, 'RIFF'); // set file size at the end

    writeUTFBytes(view, 8, 'WAVE'); // FMT sub-chunk

    writeUTFBytes(view, 12, 'fmt ');
    view.setUint32(16, 16, true);
    view.setUint16(20, 1, true); // NumChannels

    view.setUint16(22, 1, true); // SampleRate

    view.setUint32(24, this._sampleRate, true); // ByteRate

    view.setUint32(28, Number(this._sampleRate) * 1 * WAV_BITS_PER_SAMPLE / 8, true); // BlockAlign

    view.setUint16(32, 1 * Number(WAV_BITS_PER_SAMPLE) / 8, true);
    view.setUint16(34, WAV_BITS_PER_SAMPLE, true); // data sub-chunk

    writeUTFBytes(view, 36, 'data'); // file length

    view.setUint32(4, 32 + dataLength, true); // data chunk length
  ...
  _onAudioProcess(e) {
    ...
    const channelLeft = e.inputBuffer.getChannelData(0); // Need to copy the Float32Array:
  ...
  _exportMonoWAV(buffers, length) {
    ...
    const view = new DataView(buffer); // copy WAV header data into the array buffer
...
function writeUTFBytes(view, offset, string) {
  const lng = string.length; // convert to big endianness
