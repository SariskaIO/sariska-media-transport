...
export default function Moderator(roomName, xmpp, emitter, options) {
  ...
  this.options = options; // Whether SIP gateway (jigasi) support is enabled. This is set
  ...
  function listener(event) {
    if (event.data && event.data.sessionId) {
      ...
      Settings.sessionId = event.data.sessionId; // After popup is closed we will authenticate
    }
  } // Register
...
Moderator.prototype.createConferenceIq = function () {
  ...
  const callstatsDisabled = !callStatsID || !callStatsSecret || !enableCallStats // Even though AppID and AppSecret may be specified, the integration
  // of callstats.io may be disabled because of globally-disallowed
  // requests to any third parties.
  || disableThirdPartyRequests === true; // since the default is true across all the server-side components, only signal if false.
...
Moderator.prototype.parseConfigOptions = function (resultIq) {
  ...
  this.eventEmitter.emit(AuthenticationEvents.IDENTITY_UPDATED, authenticationEnabled, authIdentity); // Check if jicofo has jigasi support enabled.
