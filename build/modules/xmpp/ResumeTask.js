...
export default class ResumeTask {
  ...
  schedule() {
    ...
    this._networkOnlineListener = NetworkInfo.addEventListener(NETWORK_INFO_EVENT, ({
      isOnline
    }) => {
      if (isOnline) {
  ...
  _resumeConnection() {
    ...
    const resumeToken = streamManagement.getResumeToken(); // Things may have changed since when the task was scheduled
    ...
    const oldToken = search.match(pattern); // Replace previd if the previd value has changed.

    if (oldToken && oldToken.indexOf(resumeToken) === -1) {
      search = search.replace(pattern, `$1${resumeToken}`); // Append previd if it doesn't exist.
    } else if (!oldToken) {
