...
export default class XmppConnection extends Listenable {
  ...
  constructor({
    ...
    this._deferredIQs = []; // Ping plugin is mandatory for the Websocket mode to work correctly. It's used to detect when the connection
  ...
  get connected() {
    ...
    return (this._status === Strophe.Status.CONNECTED || this._status === Strophe.Status.ATTACHED) && (!this.isUsingWebSocket || websocket && websocket.readyState === WebSocket.OPEN);
  }
  /**
  ...
  get pingDomain() {
    var _this$_options$pingOp;

    return ((_this$_options$pingOp = this._options.pingOptions) === null || _this$_options$pingOp === void 0 ? void 0 : _this$_options$pingOp.domain) || this.domain;
  }
  /**
  ...
  set shard(value) {
    this._options.shard = value; // shard setting changed so let's schedule a new keep-alive check if connected
  ...
  _stropheConnectionCb(targetCallback, status, ...args) {
    ...
    if (status === Strophe.Status.CONNECTED || status === Strophe.Status.ATTACHED) {
      var _this$_options$pingOp2;

      this._maybeEnableStreamResume(); // after connecting - immediately check whether shard changed,
      ...
      this.ping.startInterval(((_this$_options$pingOp2 = this._options.pingOptions) === null || _this$_options$pingOp2 === void 0 ? void 0 : _this$_options$pingOp2.domain) || this.domain);
    } else if (status === Strophe.Status.DISCONNECTED) {
      this.ping.stopInterval(); // FIXME add RECONNECTING state instead of blocking the DISCONNECTED update
      ...
      if (this.connected) {
        ...
        const deferred = {
          ...
          timeout: setTimeout(() => {
            ...
            deferred.iq = undefined; // Strophe calls with undefined on timeout
