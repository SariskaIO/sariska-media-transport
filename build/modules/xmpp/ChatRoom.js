...
export default class ChatRoom extends Listenable {
  ...
  sendPresence(fromJoin) {
    ...
    parser.json2packet(this.presMap.nodes, pres); // we store time we last synced presence state
  ...
  onPresence(pres) {
    ...
    member.isHiddenDomain = jid && jid.indexOf('@') > 0 && this.options.hiddenDomain && this.options.hiddenDomain.indexOf(jid.substring(jid.indexOf('@') + 1, jid.indexOf('/'))) >= 0;
    ...
    if (from === this.myroomjid) {
      ...
      if (!this.joined) {
        ...
        this.eventEmitter.emit(XMPPEvents.MUC_JOINED); // Now let's check the disco-info to retrieve the
        // meeting Id if any
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      switch (node.tagName) {
        case 'nick':
          if (!member.isFocus) {
            const displayName = this.xmpp.options.displayJids ? Strophe.getResourceFromJid(from) : member.nick;
        ...
        case 'conference-properties':
          if (member.isFocus) {
            ...
            this.eventEmitter.emit(XMPPEvents.CONFERENCE_PROPERTIES_CHANGED, properties); // Log if Jicofo supports restart by terminate only once. This conference property does not change
            // during the call.
  ...
  onPresenceUnavailable(pres, from) {
    ...
    if (isKick) {
      ...
      let reason;
      const reasonSelect = $(pres).find('>x[xmlns="http://jabber.org/protocol/muc#user"]' + '>item>reason');
  ...
  onMessage(msg, from) {
    ...
    if (from === this.roomjid) {
      ...
      if ($(msg).find('>x[xmlns="http://jabber.org/protocol/muc#user"]>status[code="104"]').length) {
        this.discoRoomInfo();
      } else if ((invite = $(msg).find('>x[xmlns="http://jabber.org/protocol/muc#user"]>invite')) && invite.length) {
  ...
  onPresenceError(pres, from) {
    ...
    } else if ($(pres).find('>error[type="auth"]' + '>registration-required[' + 'xmlns="urn:ietf:params:xml:ns:xmpp-stanzas"]').length) {
  ...
  lockRoom(key, onSuccess, onError, onNotSupported) {
    ...
    this.connection.sendIQ($iq({
      ...
      if ($(res).find('>query>x[xmlns="jabber:x:data"]' + '>field[var="muc#roomconfig_roomsecret"]').length) {
        ...
        }).c('value').t(key === null || key.length === 0 ? '0' : '1').up().up(); // if members only enabled
  ...
  setMembersOnly(enabled, onSuccess, onError) {
    ...
    this.xmpp.connection.sendIQ($iq({
      ...
      if ($(res).find('>query>x[xmlns="jabber:x:data"]>field[var="muc#roomconfig_membersonly"]').length) {
        ...
        formToSubmit.c('field', {
          'var': 'FORM_TYPE'
        }).c('value').t('http://jabber.org/protocol/muc#roomconfig').up().up();
        formToSubmit.c('field', {
          'var': 'muc#roomconfig_membersonly'
        }).c('value').t(enabled ? 'true' : 'false').up().up(); // if room is locked from other participant or we are locking it
  ...
  addOrReplaceInPresence(key, values) {
    values.tagName = key;
    const matchingNodes = this.presMap.nodes.filter(node => key === node.tagName); // if we have found just one, let's check is it the same
  ...
  addAudioInfoToPresence(mute) {
    const audioMutedTagName = 'audiomuted'; // we skip adding it as muted is default value
  ...
  addVideoInfoToPresence(mute) {
    const videoMutedTagName = 'videomuted'; // we skip adding it as muted is default value
  ...
  leave() {
    ...
    const promises = [];
    ((_this$lobby = this.lobby) === null || _this$lobby === void 0 ? void 0 : _this$lobby.lobbyRoom) && promises.push(this.lobby.leave());
