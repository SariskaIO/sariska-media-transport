...
function createContentModify(senders = 'both', maxFrameHeight) {
  const modifyContentsIq = jQuery.parseXML('<jingle action="content-modify" initiator="peer2" sid="sid12345" xmlns="urn:xmpp:jingle:1">' + `<content name="video" senders="${senders}">` + `<max-frame-height xmlns="http://jitsi.org/jitmeet/video">${maxFrameHeight}</max-frame-height>` + '</content>' + '</jingle>');
...
describe('JingleSessionPC', () => {
  ...
  beforeEach(() => {
    ...
    jingleSession = new JingleSessionPC(SID, 'peer1', 'peer2', connection, {}, {}, true, false);
    rtc = new MockRTC();
  ...
  describe('send/receive video constraints', () => {
    it('sends content-modify with recv frame size', () => {
      ...
      return new Promise((resolve, reject) => {
        jingleSession.acceptOffer(offerIQ, resolve, reject,
        /* local tracks */
        []);
      }).then(() => {
        expect(jingleSession.getState()).toBe(JingleSessionState.ACTIVE); // FIXME content-modify is sent before session-accept

        expect(sendIQSpy.calls.count()).toBe(2);
        expect(sendIQSpy.calls.first().args[0].toString()).toBe('<iq to="peer2" type="set" xmlns="jabber:client">' + '<jingle action="content-modify" initiator="peer2" sid="sid12345" xmlns="urn:xmpp:jingle:1">' + '<content name="video" senders="both">' + '<max-frame-height xmlns="http://jitsi.org/jitmeet/video">180</max-frame-height>' + '</content>' + '</jingle>' + '</iq>');
      });
    });
    it('fires an event when remote peer sends content-modify', () => {
      ...
      jingleSession.addListener(MediaSessionEvents.REMOTE_VIDEO_CONSTRAINTS_CHANGED, remoteVideoConstraintsListener);
      return new Promise((resolve, reject) => {
        jingleSession.acceptOffer(offerIQ, resolve, reject,
        /* local tracks */
        []);
