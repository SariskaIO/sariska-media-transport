...
import ChatRoom, { parser } from './ChatRoom'; // This rule makes creating the xml elements take up way more
...
describe('ChatRoom', () => {
  describe('packet2JSON', () => {
    ...
    it('translates attributes correctly', () => {
      ...
      const fakeWithAttr = nodes.find(n => n.tagName === 'fake-with-attr');
    ...
    it('translates elements with children correctly', () => {
      ...
      {
        const user = identity.children.find(n => n.tagName === 'user');
        expect(user).toBeTruthy();
        expect(Object.keys(user.attributes).length).toEqual(0);
        expect(user.children.length).toEqual(3);
        {
          const id = user.children.find(n => n.tagName === 'id');
        ...
        {
          const name = user.children.find(n => n.tagName === 'name');
        ...
        {
          const avatar = user.children.find(n => n.tagName === 'avatar');
      ...
      {
        const group = identity.children.find(n => n.tagName === 'group');
  ...
  describe('sendMessage', () => {
    ...
    beforeEach(() => {
      ...
      room = new ChatRoom( // eslint-disable-next-line no-empty-function
      {
        send: () => {}
      }
      /* connection */
    ...
    it('sends a string msg with elementName body correctly', () => {
      ...
      expect(connectionSpy.calls.argsFor(0).toString()).toBe('<message to="jid" type="groupchat" xmlns="jabber:client">' + '<body>string message</body>' + '</message>');
    });
    it('sends a object msg with elementName body correctly', () => {
      room.sendMessage({
        object: 'message'
      }, 'body', 'receiver');
      expect(connectionSpy.calls.argsFor(0).toString()).toBe('<message to="jid" type="groupchat" xmlns="jabber:client">' + '<body object="message"/>' + '</message>');
    });
    it('sends a string msg with elementName json-message correctly', () => {
      room.sendMessage('string message', 'json-message', 'receiver');
      expect(connectionSpy.calls.argsFor(0).toString()).toBe('<message to="jid" type="groupchat" xmlns="jabber:client">' + '<json-message xmlns="http://jitsi.org/jitmeet">string message</json-message>' + '</message>');
    });
    it('sends a object msg with elementName json-message correctly', () => {
      room.sendMessage({
        object: 'message'
      }, 'json-message', 'receiver');
      expect(connectionSpy.calls.argsFor(0).toString()).toBe('<message to="jid" type="groupchat" xmlns="jabber:client">' + '<json-message object="message" xmlns="http://jitsi.org/jitmeet"/>' + '</message>');
    });
  });
});