...
export default class SignalingLayerImpl extends SignalingLayer {
  ...
  _addLocalSourceInfoToPresence() {
    if (this.chatRoom) {
      this.chatRoom.addOrReplaceInPresence(SOURCE_INFO_PRESENCE_ELEMENT, {
  ...
  _doesEndpointSendNewSourceInfo(endpointId) {
    ...
    const presence = (_this$chatRoom = this.chatRoom) === null || _this$chatRoom === void 0 ? void 0 : _this$chatRoom.getLastPresence(endpointId);
  ...
  setChatRoom(room) {
    ...
    if (oldChatRoom) {
      ...
      if (FeatureFlags.isSourceNameSignalingEnabled()) {
        ...
        this._memberLeftHandler && oldChatRoom.removeEventListener(XMPPEvents.MUC_MEMBER_LEFT, this._memberLeftHandler);
      }
    }
  ...
  _bindChatRoomEventHandlers(room) {
    ...
    this._sourceInfoHandler = (node, mucNick) => {
      ...
      for (const sourceName of Object.keys(sourceInfoJSON)) {
        ...
        const oldSourceState = endpointSourceState[sourceName] || (endpointSourceState[sourceName] = {
          sourceName
        });
    ...
    room.addPresenceListener('SourceInfo', this._sourceInfoHandler); // Cleanup when participant leaves
  ...
  getPeerMediaInfo(owner, mediaType) {
    ...
    if (FeatureFlags.isSourceNameSignalingEnabled()) {
      ...
      const endpointMediaSource = this._findEndpointSourceInfoForMediaType(owner, mediaType); // The defaults are provided only, because getPeerMediaInfo is a legacy method. This will be eventually
  ...
  setTrackVideoType(sourceName, videoType) {
    ...
    if (this._localSourceState[sourceName].videoType !== videoType) {
      ...
      this._localSourceState[sourceName].videoType = videoType === VideoType.CAMERA ? undefined : videoType; // NOTE this doesn't send the actual presence, because is called from the same place where the legacy video
