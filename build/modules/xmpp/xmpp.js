...
export default class XMPP extends Listenable {
  ...
  constructor(options, token) {
    ...
    const xmppPing = options.xmppPing || {}; // let's ping the main domain (in case a guest one is used for the connection)
    ...
    this.connection = createConnection({
      ...
      xmppPing,
      shard: (_options$deploymentIn = options.deploymentInfo) === null || _options$deploymentIn === void 0 ? void 0 : _options$deploymentIn.shard
  ...
  connectionHandler(credentials = {}, status, msg) {
    ...
    if (status === Strophe.Status.CONNECTED || status === Strophe.Status.ATTACHED) {
      ...
      this.sendDiscoInfo && this.caps.getFeaturesAndIdentities(this.options.hosts.domain).then(({
        ...
        this._processDiscoInfoIdentities(identities, undefined
        /* when querying we will query for features */
        );
      }).catch(error => {
      ...
      const lastFailedRawMessage = this.getConnection().getLastFailedMessage(); // wrong password or username, prompt user
  ...
  _processDiscoInfoIdentities(identities, features) {
    ...
    identities.forEach(identity => {
      ...
      if (identity.type === 'lobbyrooms') {
        ...
        if (features) {
          ...
          identity.name && this.caps.getFeaturesAndIdentities(identity.name, identity.type).then(({
            features: f
    ...
    if (this.avModerationComponentAddress || this.speakerStatsComponentAddress || this.conferenceDurationComponentAddress || this.breakoutRoomsComponentAddress) {
  ...
  */
  ...
  _connect(jid, password) {
    ...
    this._resetState(); // we want to send this only on the initial connect
