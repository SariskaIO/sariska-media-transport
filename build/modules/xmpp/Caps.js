...
export function parseDiscoInfo(node) {
  ...
  $(node).find('>query>feature').each((_, el) => features.add(el.getAttribute('var')));
  $(node).find('>query>identity').each((_, el) => identities.add({
...
export default class Caps extends Listenable {
  ...
  constructor(connection = {}, node = 'http://jitsi.org/jitsimeet') {
    ...
    this.version = '';
    this.rooms = new Set(); // We keep track of features added outside the library and we publish them
  ...
  _updateRoomWithExternalFeatures(room) {
    if (this.externalFeatures.size === 0) {
      ...
      room.addOrReplaceInPresence('features', {
        children
      });
    }
  }
