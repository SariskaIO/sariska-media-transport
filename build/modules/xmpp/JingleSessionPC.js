...
export default class JingleSessionPC extends JingleSession {
  ...
  doInitialize(options) {
    ...
    if (this.isP2P) {
      ...
      var _options$desktopShari, _options$desktopShari2;

      // H264 does not support simulcast, so it needs to be disabled.
      pcOptions.disableSimulcast = options.disableSimulcast || options.preferH264 && !options.disableH264 || options.videoQuality && options.videoQuality.preferredCodec === CodecMimeType.H264; // Disable simulcast for low fps screenshare and enable it for high fps screenshare.
      // testing.capScreenshareBitrate config.js setting has now been deprecated.

      pcOptions.capScreenshareBitrate = pcOptions.disableSimulcast || !(typeof ((_options$desktopShari = options.desktopSharingFrameRate) === null || _options$desktopShari === void 0 ? void 0 : _options$desktopShari.max) === 'number' && ((_options$desktopShari2 = options.desktopSharingFrameRate) === null || _options$desktopShari2 === void 0 ? void 0 : _options$desktopShari2.max) > SS_DEFAULT_FRAME_RATE); // add the capScreenshareBitrate to the permanent properties so that it's included with every event that we
    ...
    this.peerconnection.oniceconnectionstatechange = () => {
      ...
      switch (this.peerconnection.iceConnectionState) {
        ...
        case 'connected':
          ...
          if (!this.wasConnected && (this.wasstable || isStable || this.usesUnifiedPlan && this.isInitiator && browser.isChromiumBased())) {
            Statistics.sendAnalytics(ICE_DURATION, {
              phase: 'checking',
  ...
  sendIceCandidate(candidate) {
    ...
    if (candidate && candidate.candidate.length && !this.lasticecandidate) {
      ...
      logger.log(`${this} sendIceCandidate: last candidate`); // FIXME: remember to re-think in ICE-restart
    ...
    for (let mid = 0; mid < localSDP.media.length; mid++) {
      ...
      if (cands.length > 0) {
        ...
        for (let i = 0; i < cands.length; i++) {
          const candidate = SDPUtil.candidateToJingle(cands[i].candidate); // Mangle ICE candidate if 'failICE' test option is enabled
    ...
    this.setOfferAnswerCycle(jingleAnswer, () => {
      ...
      if (this.usesUnifiedPlan && browser.isChromiumBased()) {
        const workFunction = finishedCallback => {
          ...
          return this._responderRenegotiate(remoteDescription).then(() => finishedCallback(), error => finishedCallback(error));
        };
  ...
  setOfferAnswerCycle(jingleOfferAnswerIq, success, failure, localTracks = []) {
    const workFunction = finishedCallback => {
      ...
      Promise.all(addTracks).then(() => this._renegotiate(newRemoteSdp.raw)).then(() => {
        if (this.state === JingleSessionState.PENDING) {
          this.state = JingleSessionState.ACTIVE; // #1 Sync up video transfer active/inactive only after
          ...
          if (this.isP2P && (!this._localVideoActive || this.localRecvMaxFrameHeight)) {
  ...
  setVideoCodecs(preferred = null, disabled = null) {
    ...
    if (this._assertNotEnded() && preferred !== current) {
      ...
      this.peerconnection.setVideoCodecs(preferred, disabled); // Initiate a renegotiate for the codec setting to take effect.
      ...
      logger.debug(`${this} Queued setVideoCodecs task`); // Queue and execute
  ...
  replaceTransport(jingleOfferElem, success, failure) {
    ...
    this.room.eventEmitter.emit(XMPPEvents.ICE_RESTARTING, this); // We need to first reject the 'data' section to have the SCTP stack
  ...
  sendContentModify() {
    ...
    if (typeof maxFrameHeight !== 'undefined') {
      sessionModify = sessionModify.c('max-frame-height', {
  ...
  setSenderVideoConstraint(maxFrameHeight) {
    if (this._assertNotEnded()) {
      logger.info(`${this} setSenderVideoConstraint: ${maxFrameHeight}`); // RN doesn't support RTCRtpSenders yet, aggresive layer suspension on RN is implemented
  ...
  _addOrRemoveRemoteStream(isAdd, elem) {
    ...
    logger.debug(`${this} Queued ${logPrefix} task`); // Queue and execute
  ...
  _processRemoteRemoveSource(removeSsrcInfo) {
    const remoteSdp = this.usesUnifiedPlan ? new SDP(this.peerconnection.peerconnection.remoteDescription.sdp) : new SDP(this.peerconnection.remoteDescription.sdp);
        ...
        lines.forEach(line => {
          ...
          if (mid > -1) {
            ...
            if (this.isP2P) {
              ...
              const port = (_SDPUtil$parseMLine2 = SDPUtil.parseMLine(remoteSdp.media[mid].split('\r\n')[0])) === null || _SDPUtil$parseMLine2 === void 0 ? void 0 : _SDPUtil$parseMLine2.port;
              remoteSdp.media[mid] = remoteSdp.media[mid].replace(`${line}\r\n`, '');
              remoteSdp.media[mid] = remoteSdp.media[mid].replace(`m=${mediaType} ${port}`, `m=${mediaType} 0`);
            }
          }
        });
      } else {
  ...
  _processRemoteAddSource(addSsrcInfo) {
    ...
    addSsrcInfo.forEach((lines, idx) => {
      remoteSdp.media[idx] += lines; // Make sure to change the direction to 'sendrecv/sendonly' only for p2p connections. For jvb connections,
      // a new m-line is added for the new remote sources.

      if (this.isP2P && this.usesUnifiedPlan) {
        var _SDPUtil$parseMLine3;

        const mediaType = (_SDPUtil$parseMLine3 = SDPUtil.parseMLine(remoteSdp.media[idx].split('\r\n')[0])) === null || _SDPUtil$parseMLine3 === void 0 ? void 0 : _SDPUtil$parseMLine3.media;
  ...
  _initiatorRenegotiate(remoteDescription) {
    ...
    return this.peerconnection.createOffer(this.mediaConstraints).then(offer => {
      ...
      return this.peerconnection.setLocalDescription(offer).then(() => {
        logger.debug(`${this} Renegotiate: setting remote description`); // eslint-disable-next-line max-len
  ...
  addTrack(localTrack) {
    if (!FeatureFlags.isMultiStreamSupportEnabled() || !this.usesUnifiedPlan || localTrack.type !== MediaType.VIDEO) {
      return Promise.reject(new Error('Multiple tracks of a given media type are not supported'));
    }

    const workFunction = finishedCallback => {
      const remoteSdp = new SDP(this.peerconnection.peerconnection.remoteDescription.sdp); // Add a new transceiver by adding a new mline in the remote description.

      remoteSdp.addMlineForNewLocalSource(MediaType.VIDEO);

      this._renegotiate(remoteSdp.raw).then(() => finishedCallback(), error => finishedCallback(error));
    };
    ...
    return new Promise((resolve, reject) => {
      ...
      this.modificationQueue.push(workFunction, error => {
        if (error) {
          ...
          logger.debug(`${this} renegotiation after addTrack executed - OK`); // Replace the track on the newly generated transceiver.

          return this.replaceTrack(null, localTrack).then(() => resolve()).catch(() => reject());
        }
      });
    });
  }
  ...
  setMediaTransferActive(audioActive, videoActive) {
    ...
    const workFunction = finishedCallback => {
      const isSessionActive = this.state === JingleSessionState.ACTIVE; // Because the value is modified on the queue it's impossible to
  ...
  modifyContents(jingleContents) {
    ...
    const newMaxFrameHeight = JingleSessionPC.parseMaxFrameHeight(jingleContents); // frame height is optional in our content-modify protocol
  ...
  notifyMySSRCUpdate(oldSDP, newSDP) {
    ...
    const getSignaledSourceInfo = sdpDiffer => {
      ...
      let ssrcs = [];
      let mediaType = null; // It is assumed that sources are signaled one at a time.
    ...
    const removedAnySSRCs = sdpDiffer.toJingle(remove); // context a common object for one run of ssrc update (source-add and source-remove) so we can match them if we
    // need to

    const ctx = {};

    if (removedAnySSRCs) {
      const sourceInfo = getSignaledSourceInfo(sdpDiffer); // Log only the SSRCs instead of the full IQ.
  ...
  newJingleErrorHandler(request, failureCb) {
    return errResponse => {
      ...
      if (failureCb) {
        failureCb(error);
      } else if (this.state === JingleSessionState.ENDED && error.reason === 'item-not-found') {
  ...
  close() {
    ...
    logger.debug(`${this} Clearing modificationQueue`); // Remove any pending tasks from the queue
    ...
    logger.debug(`${this} Shutdown modificationQueue!`); // No more tasks can go in after the close task
