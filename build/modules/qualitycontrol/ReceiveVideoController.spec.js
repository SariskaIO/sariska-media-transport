...
import { ReceiveVideoController } from './ReceiveVideoController'; // JSDocs disabled for Mock classes to avoid duplication - check on the original classes for info.
  ...
  describe('when sourceNameSignaling is enabled', () => {
    ...
    it('should not allow the endpoints format.', () => {
      ...
      try {
        ...
        expect(error).toEqual(new Error('"onStageEndpoints" and "selectedEndpoints" are not supported when sourceNameSignaling is enabled.'));
      }
    });
  });
  describe('when sourceNameSignaling is disabled', () => {
    ...
    it('should not allow the source names format.', () => {
      ...
      try {
        ...
        expect(error).toEqual(new Error('"onStageSources" and "selectedSources" are not supported when sourceNameSignaling is disabled.'));
      }
    });
  });
});