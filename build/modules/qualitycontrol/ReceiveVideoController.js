...
export class ReceiverVideoConstraints {
  ...
  constructor() {
    ...
    this._defaultConstraints = {
      'maxHeight': MAX_HEIGHT_THUMBNAIL
    }; // The number of videos requested from the bridge.

    this._lastN = LASTN_UNLIMITED; // The number representing the maximum video height the local client should receive from the bridge.

    this._maxFrameHeight = MAX_HEIGHT_ONSTAGE; // The endpoint IDs of the participants that are currently selected.
...
export class ReceiveVideoController {
  ...
  constructor(conference, rtc) {
    var _config$startLastN, _config$useNewBandwid;

    this._conference = conference;
    this._rtc = rtc;
    const {
      config
    } = conference.options; // The number of videos requested from the bridge, -1 represents unlimited or all available videos.

    this._lastN = (_config$startLastN = config === null || config === void 0 ? void 0 : config.startLastN) !== null && _config$startLastN !== void 0 ? _config$startLastN : (config === null || config === void 0 ? void 0 : config.channelLastN) || LASTN_UNLIMITED; // The number representing the maximum video height the local client should receive from the bridge.

    this._maxFrameHeight = MAX_HEIGHT_ONSTAGE; // Enable new receiver constraints by default unless it is explicitly disabled through config.js.

    const useNewReceiverConstraints = (_config$useNewBandwid = config === null || config === void 0 ? void 0 : config.useNewBandwidthAllocationStrategy) !== null && _config$useNewBandwid !== void 0 ? _config$useNewBandwid : true;
  ...
  selectEndpoints(ids) {
    ...
    if (this._receiverVideoConstraints) {
      ...
      const newConstraints = this._receiverVideoConstraints.constraints; // Send bridge message only when the constraints change.
  ...
  setLastN(value) {
    if (this._lastN !== value) {
      ...
      if (this._receiverVideoConstraints) {
        const lastNUpdated = this._receiverVideoConstraints.updateLastN(value); // Send out the message on the bridge channel if lastN was updated.
  ...
  setPreferredReceiveMaxFrameHeight(maxFrameHeight) {
    ...
    for (const session of this._conference._getMediaSessions()) {
      if (session.isP2P || !this._receiverVideoConstraints) {
        ...
        resolutionUpdated && this._rtc.setNewReceiverVideoConstraints(this._receiverVideoConstraints.constraints);
      }
    }
  }
  /**
  ...
  setReceiverConstraints(constraints) {
    ...
    if (constraintsChanged) {
      var _constraints$lastN, _constraints$selected;

      this._lastN = (_constraints$lastN = constraints.lastN) !== null && _constraints$lastN !== void 0 ? _constraints$lastN : this._lastN;
      this._selectedEndpoints = (_constraints$selected = constraints.selectedEndpoints) !== null && _constraints$selected !== void 0 ? _constraints$selected : this._selectedEndpoints;
      ...
      if (p2pSession) {
        ...
        let maxFrameHeight = (_Object$values$ = Object.values(constraints.constraints)[0]) === null || _Object$values$ === void 0 ? void 0 : _Object$values$.maxHeight;

        if (!maxFrameHeight) {
          var _constraints$defaultC;

          maxFrameHeight = (_constraints$defaultC = constraints.defaultConstraints) === null || _constraints$defaultC === void 0 ? void 0 : _constraints$defaultC.maxHeight;
        }
