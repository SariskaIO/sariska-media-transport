...
import { SendVideoController } from './SendVideoController'; // JSDocs disabled for Mock classes to avoid duplication - check on the original classes for info.
...
class MockJingleSessionPC extends Listenable {
  ...
  setRemoteRecvMaxFrameHeight(remoteRecvMaxFrameHeight) {
    ...
    if (this.isP2P) {
      ...
      this.rtc.eventEmitter.emit(RTCEvents.SENDER_VIDEO_CONSTRAINTS_CHANGED, {
...
describe('SendVideoController', () => {
  ...
  beforeEach(() => {
    ...
    jvbConnection = new MockJingleSessionPC(rtc, false
    /* isP2P */
    );
    p2pConnection = new MockJingleSessionPC(rtc, true
    /* isP2P */
    );
