...
describe('SDP', () => {
  describe('toJingle', () => {
    ...
    it('correctly groups ssrcs lines that are not in order', () => {
      ...
      const descriptions = Array.from(nodeTree.getElementsByTagName('description'));
      const videoDescriptions = descriptions.filter(description => description.getAttribute('media') === 'video');
      const count = videoDescriptions.reduce((iterator, description) => {
        const childNodes = Array.from(description.childNodes);
        const childNodesSources = childNodes.filter(child => child.nodeName === 'source');
