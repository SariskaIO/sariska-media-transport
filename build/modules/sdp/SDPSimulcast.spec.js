...
const numVideoSsrcs = parsedSdp => {
  var _videoMLine$ssrcs;

  const videoMLine = parsedSdp.media.find(m => m.type === MediaType.VIDEO);
  const ssrcs = new Set((_videoMLine$ssrcs = videoMLine.ssrcs) === null || _videoMLine$ssrcs === void 0 ? void 0 : _videoMLine$ssrcs.map(ssrcInfo => ssrcInfo.id));
  return ssrcs.size;
};

const parseSimLayers = parsedSdp => {
  var _videoMLine$ssrcGroup;

  const videoMLine = parsedSdp.media.find(m => m.type === MediaType.VIDEO);
  const simGroup = (_videoMLine$ssrcGroup = videoMLine.ssrcGroups) === null || _videoMLine$ssrcGroup === void 0 ? void 0 : _videoMLine$ssrcGroup.find(group => group.semantics === 'SIM');
...
describe('sdp-simulcast', () => {
  ...
  describe('mungeLocalDescription', () => {
    ...
    it('should add the cached SSRCs on subsequent sLD calls to the local sdp', () => {
      ...
      const newDesc = simulcast.mungeLocalDescription(desc);
      const newSdp = transform.parse(newDesc.sdp);
      const cachedSsrcs = parseSimLayers(newSdp); // Call sLD again with the original description.
