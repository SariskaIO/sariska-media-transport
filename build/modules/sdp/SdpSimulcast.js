...
export default class SdpSimulcast {
  ...
  _getSsrcAttribute(mLine, ssrc, attributeName) {
    var _mLine$ssrcs, _mLine$ssrcs$find;

    return (_mLine$ssrcs = mLine.ssrcs) === null || _mLine$ssrcs === void 0 ? void 0 : (_mLine$ssrcs$find = _mLine$ssrcs.find(ssrcInfo => Number(ssrcInfo.id) === ssrc && ssrcInfo.attribute === attributeName)) === null || _mLine$ssrcs$find === void 0 ? void 0 : _mLine$ssrcs$find.value;
  }
  /**
  ...
  _parseSimLayers(mLine) {
    var _mLine$ssrcGroups, _mLine$ssrcs2;

    const simGroup = (_mLine$ssrcGroups = mLine.ssrcGroups) === null || _mLine$ssrcGroups === void 0 ? void 0 : _mLine$ssrcGroups.find(group => group.semantics === 'SIM');

    if (simGroup) {
      return simGroup.ssrcs.split(' ').map(ssrc => Number(ssrc));
    }

    if ((_mLine$ssrcs2 = mLine.ssrcs) === null || _mLine$ssrcs2 === void 0 ? void 0 : _mLine$ssrcs2.length) {
  ...
  mungeLocalDescription(description) {
    ...
    for (let media of session.media) {
      var _media$ssrcs, _media$ssrcGroups$len, _media$ssrcGroups;
      ...
      const mid = media.mid;
      const numSsrcs = new Set((_media$ssrcs = media.ssrcs) === null || _media$ssrcs === void 0 ? void 0 : _media$ssrcs.map(ssrcInfo => ssrcInfo.id));
      const numGroups = (_media$ssrcGroups$len = (_media$ssrcGroups = media.ssrcGroups) === null || _media$ssrcGroups === void 0 ? void 0 : _media$ssrcGroups.length) !== null && _media$ssrcGroups$len !== void 0 ? _media$ssrcGroups$len : 0;
      let primarySsrc; // Do not munge if the description has no ssrcs or if simulcast is already enabled.
      ...
      if (numSsrcs.size === 1) {
        ...
        primarySsrc = Number((_media$ssrcs$ = media.ssrcs[0]) === null || _media$ssrcs$ === void 0 ? void 0 : _media$ssrcs$.id);
      } else {
      ...
      if (this._ssrcCache.has(mid)) {
        ...
        const simulcastSsrcs = this._parseSimLayers(media); // Update the SSRCs in the cache so that they can re-used for the same mid again.
  ...
  mungeRemoteDescription(description) {
    ...
    for (const media of session.media) {
      ...
      if (!((_media$ssrcGroups2 = media.ssrcGroups) === null || _media$ssrcGroups2 === void 0 ? void 0 : _media$ssrcGroups2.length) || !(media === null || media === void 0 ? void 0 : media.ssrcs.length)) {
        continue;
      ...
      if (fidGroups.length && simGroup) {
        ...
        for (const ssrc of media.ssrcs) {
          if (ssrc.id.toString() !== primarySsrc && ssrc.id.toString() !== secondarySsrc) {
