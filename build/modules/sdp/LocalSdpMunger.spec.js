...
function getSsrcLines(desc, mediaType) {
  ...
  return (_mline$ssrcs = mline.ssrcs) !== null && _mline$ssrcs !== void 0 ? _mline$ssrcs : [];
}
  ...
  describe('stripSsrcs', () => {
    ...
    describe('should do nothing to an sdp with msid', () => {
      ...
      it('with source name signaling enabled (injected source name)', () => {
        ...
        transformStreamIdentifiers();
        expect(audioSsrcs.length).toEqual(4 + 1
        /* injected source name */
        );
        expect(videoSsrcs.length).toEqual(6 + 3
        /* injected source name into each ssrc */
        );
      });
    });
  });
  ...
  describe('stripSsrcs', () => {
    describe('should not strip ssrcs from an sdp with no msid', () => {
      ...
      it('with source name signaling', () => {
        ...
        transformStreamIdentifiers();
        expect(audioSsrcs.length).toEqual(1 + 1
        /* injected source name */
        );
        expect(videoSsrcs.length).toEqual(1 + 1
        /* injected source name */
        );
      });
    });
