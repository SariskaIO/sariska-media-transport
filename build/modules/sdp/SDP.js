...
SDP.prototype.addMlineForNewLocalSource = function (mediaType) {
  ...
  const mline = clonedeep(sdp.media.find(m => m.type === mediaType)); // Edit media direction, mid and remove the existing ssrc lines in the m-line.

  mline.mid = mid;
  mline.direction = MediaDirection.RECVONLY; // Remove the ssrcs and source groups.

  mline.msid = undefined;
  mline.ssrcs = undefined;
  mline.ssrcGroups = undefined;
  sdp.media = sdp.media.concat(mline); // We regenerate the BUNDLE group (since we added a new m-line)
  ...
  for (let i = 0; i < this.media.length; i++) {
    ...
    if (mline.media === 'audio' || mline.media === 'video') {
      ...
      for (let j = 0; j < mline.fmt.length; j++) {
        ...
        elem.c('payload-type', SDPUtil.parseRTPMap(rtpmap)); // put any 'a=fmtp:' + mline.fmt[j] lines into <param name=foo
        // value=bar/>

        const afmtpline = SDPUtil.findLine(this.media[i], `a=fmtp:${mline.fmt[j]}`);

        if (afmtpline) {
          const fmtpParameters = SDPUtil.parseFmtp(afmtpline); // eslint-disable-next-line max-depth
      ...
      if (ssrc) {
        ...
        for (const [availableSsrc, ssrcParameters] of ssrcMap) {
          ...
          const msid = SDPUtil.parseMSIDAttribute(ssrcParameters); // eslint-disable-next-line max-depth
...
SDP.prototype.transportToJingle = function (mediaindex, elem) {
  ...
  if (iceParameters) {
    ...
    elem.attrs(iceParameters); // XEP-0176
  ...
  if (groups.length) {
    groups.each((idx, group) => {
      ...
      if (contents.length > 0) {
        this.raw += `a=group:${group.getAttribute('semantics') || group.getAttribute('type')} ${contents.join(' ')}\r\n`;
      }
    });
  }
...
SDP.prototype.jingle2media = function (content) {
  ...
  desc.find('>payload-type').each((_, payloadType) => {
    ...
    if ($(payloadType).find('>parameter').length) {
      ...
      sdp += $(payloadType).find('>parameter').map((__, parameter) => {
  ...
  sdp += this.rtcpFbFromJingle(desc, '*'); // xep-0294
  ...
  desc.find('>ssrc-group[xmlns="urn:xmpp:jingle:apps:rtp:ssma:0"]').each((_, ssrcGroup) => {
    ...
    const ssrcs = $(ssrcGroup).find('>source').map((__, source) => source.getAttribute('ssrc')).get();
  ...
  desc.find('>source[xmlns="urn:xmpp:jingle:apps:rtp:ssma:0"]').each((_, source) => {
    ...
    let sourceStr = '';
    $(source).find('>parameter').each((__, parameter) => {
      var _value;
      ...
      if ((_value = value) === null || _value === void 0 ? void 0 : _value.includes('mixedmslabel')) {
        isUserSource = false;
      }
    });
