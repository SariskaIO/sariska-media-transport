...
function numVideoSsrcs(parsedSdp) {
  ...
  return videoMLine.ssrcs.map(ssrcInfo => ssrcInfo.id).filter((ssrc, index, array) => array.indexOf(ssrc) === index).length;
}
/**
  ...
  describe('modifyRtxSsrcs', () => {
    describe('when given an sdp with a single video ssrc', () => {
      ...
      it('should add a single rtx ssrc', () => {
        ...
        expect(newPrimaryVideoSsrc).toEqual(primaryVideoSsrc); // Should now have an rtx ssrc as well
      ...
      it('should re-use the same rtx ssrc for a primary ssrc it\'s seen before', () => {
        ...
        const fidGroupRtxSsrc = SDPUtil.parseGroupSsrcs(fidGroup)[1]; // Now pass the original sdp through again
      ...
      it('should NOT re-use the same rtx ssrc for a primary ssrc it\'s seen before if the cache has been cleared', () => {
        ...
        rtxModifier.clearSsrcCache(); // Now pass the original sdp through again
    ...
    describe('when given an sdp with multiple video ssrcs', () => {
      ...
      it('should add rtx ssrcs for all of them', () => {
        ...
        expect(newPrimaryVideoSsrcs).toEqual(primaryVideoSsrcs); // Should now have rtx ssrcs as well

        expect(numVideoSsrcs(newSdp)).toEqual(primaryVideoSsrcs.length * 2); // Should now have FID groups
      ...
      it('should re-use the same rtx ssrcs for any primary ssrc it\'s seen before', () => {
        ...
        let fidGroups = getVideoGroups(newSdp, 'FID'); // Save the first mapping that is made
      ...
      it('should NOT re-use the same rtx ssrcs for any primary ssrc it\'s seen before if the cache has been cleared', () => {
        ...
        rtxModifier.clearSsrcCache(); // Now pass the original sdp through again and make sure we get the same mapping
