...
export default class _RTCPeerConnection extends RTCPeerConnection {
  ...
  setRemoteDescription(description) {
    _synthesizeIPv6Addresses(description).catch(reason => {
...
_RTCPeerConnection.prototype.setRemoteDescription = function (description) {
  return _synthesizeIPv6Addresses(description).catch(reason => {
...
function _setRemoteDescription(description) {
  return new Promise((resolve, reject) => {
    ...
    RTCPeerConnection.prototype.setRemoteDescription.call(this, description).then((...args) => {
      let q;
...
function _synthesizeIPv6Addresses0(sessionDescription) {
  ...
  do {
    ...
    if (end === -1) {
      line = sdp.substring(start); // Break out of the loop at the end of the iteration.
    ...
    if (line.startsWith('a=candidate:')) {
      ...
      if (candidate.length >= 10 && candidate[6] === 'typ') {
        ...
        for (const ip of ip4s) {
          if (ip.indexOf(':') === -1) {
            ips.has(ip) || ips.set(ip, new Promise((resolve, reject) => {
              const v = ips.get(ip);
