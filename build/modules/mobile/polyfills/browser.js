...
(global => {
  ...
  global.DOMParser = DOMParser; // addEventListener
  //
  // Required by:
  // - jQuery
  ...
  if (typeof global.document === 'undefined') {
    const document = new DOMParser().parseFromString('<html><head></head><body></body></html>', 'text/xml'); // document.addEventListener
    //
    // Required by:
    // - jQuery
    ...
    if (typeof document.implementation.createHTMLDocument === 'undefined') {
      document.implementation.createHTMLDocument = function (title = '') {
        const htmlDocument = new DOMParser().parseFromString(`<html>
    ...
    if (elementPrototype) {
      ...
      if (typeof elementPrototype.compareDocumentPosition === 'undefined') {
        elementPrototype.compareDocumentPosition = function (b) {
          let a = this;
      ...
      if (!elementPrototype.hasOwnProperty('innerHTML')) {
        Object.defineProperty(elementPrototype, 'innerHTML', {
          ...
          set(innerHTML) {
            ...
            this.textContent = ''; // Parse the content string.

            const d = new DOMParser().parseFromString(`<div>${innerHTML}</div>`, 'text/xml'); // Assign the resulting nodes as children of the
            // element.
    ...
    if (nodePrototype // XXX The intention was to find Node from which Document and
          ...
          if (typeof consoleLog === 'function') {
            console[level] = function (...args) {
              ...
              for (let i = 0; i < length; ++i) {
                ...
                if (arg && typeof arg !== 'string' // Limit the console.log override to
                // Node (instances).
  ...
  if (navigator) {
    ...
    let userAgent = navigator.userAgent || ''; // react-native/version
  ...
  global.callstats = require('react-native-callstats/callstats'); // XMLHttpRequest

  if (global.XMLHttpRequest) {
    const {
      prototype
    } = global.XMLHttpRequest; // XMLHttpRequest.responseXML
    //
    // Required by:
    // - Strophe
