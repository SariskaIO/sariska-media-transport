...
export default class Storage {
  ...
  constructor(keyPrefix) {
     ...
     * @type {string}
     */
    this._keyPrefix = keyPrefix; // Perform optional asynchronous initialization.

    const initializing = this._initializeAsync();

    if (initializing) {
      // Indicate that asynchronous initialization is under way.
      this._initializing = initializing; // When the asynchronous initialization completes, indicate its
      // completion.
  ...
  _getItemAsync(key) {
    return (this._initializing || Promise.resolve()).catch(() => {
      ...
      return new Promise(resolve => {
        AsyncStorage.getAllKeys().then((...getAllKeysCallbackArgs) => {
          ...
          AsyncStorage.multiGet(keys).then((...multiGetCallbackArgs) => {
            ...
            const keyPrefixLength = this._keyPrefix && this._keyPrefix.length; // eslint-disable-next-line prefer-const

            for (let [key, value] of result) {
              key = key.substring(keyPrefixLength); // XXX The loading of the previously persisted data
