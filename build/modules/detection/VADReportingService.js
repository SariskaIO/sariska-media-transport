...
export default class VADReportingService extends EventEmitter {
  ...
  static create(micDeviceList, intervalDelay, createVADProcessor) {
    ...
    const audioDeviceList = micDeviceList.filter(device => device.kind === 'audioinput'); // Create a TrackVADEmitter for each provided audio input device.

    for (const micDevice of audioDeviceList) {
      logger.log(`Initializing VAD context for mic: ${micDevice.label} -> ${micDevice.deviceId}`);
      const emitterPromise = createVADProcessor().then(rnnoiseProcessor => TrackVADEmitter.create(micDevice.deviceId, SCRIPT_NODE_SAMPLE_RATE, rnnoiseProcessor)).then(emitter => {
  ...
  _reportVadScore() {
    ...
    const computeTimestamp = Date.now(); // Go through each device and compute cumulated VAD score.
