...
/* global BigInt */
import { deriveKeys, importKey, ratchet } from './crypto-utils'; // We use a ringbuffer of keys so we can change them and still decode packets that were
// encrypted with an old key. We use a size of 16 which corresponds to the four bits
// in the frame trailer.

const KEYRING_SIZE = 16; // We copy the first bytes of the VP8 payload unencrypted.
...
export class Context {
  ...
  constructor(id) {
    ...
    this._cryptoKeyRing = new Array(KEYRING_SIZE); // A pointer to the currently used key.
  ...
  encodeFunction(encodedFrame, controller) {
    ...
    if (this._cryptoKeyRing[keyIndex]) {
      const iv = this._makeIV(encodedFrame.getMetadata().synchronizationSource, encodedFrame.timestamp); // Thіs is not encrypted and contains the VP8 payload descriptor or the Opus TOC byte.


      const frameHeader = new Uint8Array(encodedFrame.data, 0, UNENCRYPTED_BYTES[encodedFrame.type]); // Frame trailer contains the R|IV_LENGTH and key index

      const frameTrailer = new Uint8Array(2);
      frameTrailer[0] = IV_LENGTH;
      frameTrailer[1] = keyIndex; // Construct frame trailer. Similar to the frame header described in
      ...
      return crypto.subtle.encrypt({
      ...
      }, this._cryptoKeyRing[keyIndex].encryptionKey, new Uint8Array(encodedFrame.data, UNENCRYPTED_BYTES[encodedFrame.type])).then(cipherText => {
        ...
        console.error(e); // We are not enqueuing the frame here on purpose.
      });
    }
    /* NOTE WELL:
  ...
  async _decryptFrame(encodedFrame, keyIndex, initialKey = undefined, ratchetCount = 0) {
    const {
      encryptionKey
    } = this._cryptoKeyRing[keyIndex];
    let {
      material
    } = this._cryptoKeyRing[keyIndex]; // Construct frame trailer. Similar to the frame header described in
    // https://tools.ietf.org/html/draft-omara-sframe-00#section-4.2
    // but we put it at the end.
    //
    // ---------+-------------------------+-+---------+----
    ...
    try {
      ...
      if (ratchetCount < RATCHET_WINDOW_SIZE) {
        ...
        return await this._decryptFrame(encodedFrame, keyIndex, initialKey || this._cryptoKeyRing[this._currentKeyIndex], ratchetCount + 1);
      }
      /*
      ...
      */
      ...
      this._setKeys(initialKey); // TODO: notify the application about error status.
