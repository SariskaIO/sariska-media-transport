...
export class OlmAdapter extends Listenable {
  ...
  async initSessions() {
    if (this._sessionInitialization) {
      ...
      await Promise.allSettled(promises); // TODO: retry failed ones.
  ...
  async updateKey(key) {
    ...
    for (const participant of this._conf.getParticipants()) {
      ...
      const olmData = this._getParticipantOlmData(participant); // TODO: skip those who don't support E2EE.


      if (!olmData.session) {
        logger.warn(`Tried to send key to participant ${pId} but we have no session`); // eslint-disable-next-line no-continue
      ...
      case OLM_MESSAGE_TYPES.SESSION_INIT:
        {
          if (olmData.session) {
            ...
            olmData.session = session; // Send ACK
      ...
      case OLM_MESSAGE_TYPES.SESSION_ACK:
        {
          if (olmData.session) {
            ...
            const session = new Olm.Session();
            session.create_inbound(this._olmAccount, ciphertext.body); // Remove OT keys that have been used to setup this session.

            this._olmAccount.remove_one_time_keys(session); // Decrypt first message.
  ...
  */
  ...
  _sendSessionInit(participant) {
    ...
    this._sendMessage(init, pId); // Store the UUID for matching with the ACK.
