...
describe('SpeakerStatsCollector', () => {
  ...
  beforeEach(() => {
    ...
    mockConference.eventEmitter.emit(JitsiConferenceEvents.USER_JOINED, mockRemoteUser.id, new MockJitsiParticipant(mockRemoteUser.id, mockRemoteUser.name));
  });
  ...
  describe('on user name change', () => {
    it('updates the username', () => {
      ...
      mockConference.eventEmitter.emit(JitsiConferenceEvents.DISPLAY_NAME_CHANGED, mockRemoteUser.id, newName);
      const stats = speakerStatsCollector.getStats();
      const remoteUserStats = stats[mockRemoteUser.id];
  ...
  describe('on user leave', () => {
    it('retains the user stats but marks the user as left', () => {
      mockConference.eventEmitter.emit(JitsiConferenceEvents.USER_LEFT, mockRemoteUser.id);
      const stats = speakerStatsCollector.getStats();
      const remoteUserStats = stats[mockRemoteUser.id];
  ...
  describe('on dominant speaker change', () => {
    it('updates models to reflect the new dominant speaker', () => {
      ...
      const remoteUserStats = stats[mockRemoteUser.id];
      const currentUserStats = stats[mockMyId];
      mockConference.eventEmitter.emit(JitsiConferenceEvents.DOMINANT_SPEAKER_CHANGED, mockRemoteUser.id);
      expect(remoteUserStats.isDominantSpeaker()).toBe(true);
      expect(currentUserStats.isDominantSpeaker()).toBe(false);
      mockConference.eventEmitter.emit(JitsiConferenceEvents.DOMINANT_SPEAKER_CHANGED, mockMyId);
