      ...
      if (browser.supportsReceiverStats()) {
        ...
        for (const ssrc in audioLevels) {
          if (audioLevels.hasOwnProperty(ssrc)) {
            ...
            this.eventEmitter.emit(StatisticsEvents.AUDIO_LEVEL, this.peerconnection, Number.parseInt(ssrc, 10), audioLevel, false
            /* isLocal */
            );
          }
        }
        ...
        this.peerconnection.getStats().then(report => {
          this.currentAudioLevelsReport = typeof (report === null || report === void 0 ? void 0 : report.result) === 'function' ? report.result() : report;
  ...
  this.currentStatsReport.forEach(now => {
    ...
    if (now.type === 'candidate-pair' && now.nominated && now.state === 'succeeded') {
      ...
      if (remoteUsedCandidate && localUsedCandidate) {
        const remoteIpAddress = browser.isChromiumBased() ? remoteUsedCandidate.ip : remoteUsedCandidate.address;
        const remotePort = remoteUsedCandidate.port;
        const ip = `${remoteIpAddress}:${remotePort}`;
        const localIpAddress = browser.isChromiumBased() ? localUsedCandidate.ip : localUsedCandidate.address;
        const localPort = localUsedCandidate.port;
        const localip = `${localIpAddress}:${localPort}`;
        const type = remoteUsedCandidate.protocol; // Save the address unless it has been saved already.
      ...
      const numberOfActiveStreams = this.peerconnection.getActiveSimulcastStreams(); // Reset frame rate to 0 when video is suspended as a result of endpoint falling out of last-n.
