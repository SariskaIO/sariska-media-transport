...
class GoogleAnalyticsHandler extends AbstractHandler {
  ...
  _extractValue(event) {
    let value = event && event.attributes && event.attributes.value; // Try to extract an integer from the "value" attribute.
  ...
  setUserProperties(userProps = {}) {
    ...
    this._userPropertiesString = Object.keys(userProps).filter(key => filter.indexOf(key) === -1).map(key => `permanent_${key}=${userProps[key]}`).join('&');
  }
  /**
