...
export default class AbstractHandler {
  ...
  constructor(options = {}) {
    this._enabled = false;
    this._whiteListedEvents = options.whiteListedEvents; // FIXME:
    ...
    this._blackListedEvents = [...(options.blackListedEvents || []), // eslint-disable-line no-extra-parens
    'e2e_rtt', 'rtp.stats', 'rtt.by.region', 'available.device', 'stream.switch.delay', 'ice.state.changed', 'ice.duration', 'peer.conn.status.duration'];
  }
  /**
  ...
  _extractName(event) {
    ...
    const {
      ...
      actionSubject,
      source
    } = event; // All events have action, actionSubject, and source fields. All
