...
export default class ParticipantConnectionStatusHandler {
  ...
  _getVideoFrozenTimeout(id) {
    return this.rtc.isInLastN(id) ? this.rtcMuteTimeout : this.conference.isP2PActive() ? this.p2pRtcMuteTimeout : this.outOfLastNTimeout;
  }
  /**
  ...
  init() {
    ...
    this.rtc.addListener(RTCEvents.ENDPOINT_CONN_STATUS_CHANGED, this._onEndpointConnStatusChanged); // Handles P2P status changes

    this._onP2PStatus = this.refreshConnectionStatusForAll.bind(this);
    this.conference.on(JitsiConferenceEvents.P2P_STATUS, this._onP2PStatus); // Used to send analytics events for the participant that left the call.

    this._onUserLeft = this.onUserLeft.bind(this);
    this.conference.on(JitsiConferenceEvents.USER_LEFT, this._onUserLeft); // On some browsers MediaStreamTrack trigger "onmute"/"onunmute"
    ...
    if (browser.supportsVideoMuteOnConnInterrupted()) {
      ...
      this.rtc.addListener(RTCEvents.REMOTE_TRACK_UNMUTE, this._onTrackRtcUnmuted); // Track added/removed listeners are used to bind "mute"/"unmute"
      // event handlers

      this._onRemoteTrackAdded = this.onRemoteTrackAdded.bind(this);
      this.conference.on(JitsiConferenceEvents.TRACK_ADDED, this._onRemoteTrackAdded);
      this._onRemoteTrackRemoved = this.onRemoteTrackRemoved.bind(this);
      this.conference.on(JitsiConferenceEvents.TRACK_REMOVED, this._onRemoteTrackRemoved); // Listened which will be bound to JitsiRemoteTrack to listen for
  ...
  figureOutConnectionStatus(id) {
    ...
    const audioOnlyMode = this.conference.getLastN() === 0; // NOTE Overriding videoMuted to true for audioOnlyMode should disable
    ...
    if (!('p2p' in oldConnectionStatus) || !('connectionStatus' in oldConnectionStatus) || oldConnectionStatus.p2p !== inP2PMode || oldConnectionStatus.connectionStatus !== newState) {
      ...
      this.connectionStatusMap[id] = { ...oldConnectionStatus,
  ...
  _onLastNChanged(leavingLastN = [], enteringLastN = []) {
    const now = Date.now();
    logger.debug(`LastN endpoints changed leaving=${leavingLastN}, entering=${enteringLastN} at ${now}`); // If the browser doesn't fire the mute/onmute events when the remote peer stops/starts sending media,
  ...
  onTrackVideoTypeChanged(track, type) {
    ...
    const nowMs = Date.now();
    this.maybeSendParticipantConnectionStatusEvent(id, nowMs);
    this.connectionStatusMap[id] = { ...(this.connectionStatusMap[id] || {}),
      videoType: type,
      startedMs: nowMs
    };
  }
