...
JitsiConferenceEventManager.prototype.setupChatRoomListeners = function () {
  ...
  chatRoom.addListener(XMPPEvents.VIDEO_MUTED_BY_FOCUS, actor => {
    ...
    conference.mutedVideoByFocusActor = actor; // set isVideoMutedByFocus when setVideoMute Promise ends
  ...
  chatRoom.addListener(XMPPEvents.MUC_JOINED, () => {
    ...
    Object.keys(chatRoom.connectionTimes).forEach(key => {
      const event = createConnectionStageReachedEvent(`conference_${key}`, {
    ...
    Object.keys(chatRoom.xmpp.connectionTimes).forEach(key => {
      const event = createConnectionStageReachedEvent(`xmpp_${key}`, {
  ...
  chatRoom.addListener(XMPPEvents.SESSION_ACCEPT_TIMEOUT, jingleSession => {
    Statistics.sendAnalyticsAndLog(createJingleEvent(ACTION_JINGLE_SA_TIMEOUT, {
...
JitsiConferenceEventManager.prototype.setupRTCListeners = function () {
  ...
  rtc.addListener(RTCEvents.DOMINANT_SPEAKER_CHANGED, (dominant, previous) => {
    ...
    if (conference.lastDominantSpeaker !== dominant && conference.room) {
      ...
      if (previous && previous.length) {
        const speakerList = previous.slice(0); // Add the dominant speaker to the top of the list (exclude self).
  ...
  rtc.addListener(RTCEvents.DATA_CHANNEL_OPEN, () => {
    ...
    Statistics.sendAnalytics(createConnectionStageReachedEvent(key, {
      value: now
    }));
...
JitsiConferenceEventManager.prototype.setupXMPPListeners = function () {
  ...
  this._addConferenceXMPPListener(XMPPEvents.AV_MODERATION_APPROVED, value => conference.eventEmitter.emit(JitsiConferenceEvents.AV_MODERATION_APPROVED, {
    mediaType: value
  }));

  this._addConferenceXMPPListener(XMPPEvents.AV_MODERATION_REJECTED, value => {
    conference.eventEmitter.emit(JitsiConferenceEvents.AV_MODERATION_REJECTED, {
      mediaType: value
    });
  });
};
