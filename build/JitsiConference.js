...
JitsiConference.prototype.leave = async function () {
  ...
  try {
    ...
    leaveError = err; // Remove all participants because currently the conference
  ...
  if (localTracks.length > 0) {
    ...
    if (FeatureFlags.isMultiStreamSupportEnabled() && mediaType === MediaType.VIDEO) {
      ...
      return Promise.all(addTrackPromises).then(() => {
        this._setupNewTrack(track); // TODO Update presence and sent videoType message.
...
JitsiConference.prototype.replaceTrack = function (oldTrack, newTrack) {
  ...
  return this._doReplaceTrack(oldTrackBelongsToConference ? oldTrack : null, newTrack).then(() => {
    ...
    if (oldTrackBelongsToConference && (oldTrack === null || oldTrack === void 0 ? void 0 : oldTrack.isVideoTrack()) // we do not want to send presence update during setEffect switching, which does remove and then add
...
JitsiConference.prototype._setupNewTrack = function (newTrack) {
  ...
  if (FeatureFlags.isSourceNameSignalingEnabled() && !newTrack.getSourceName()) {
    var _this$getLocalTracks;

    const sourceName = getSourceNameForJitsiTrack(this.myUserId(), mediaType, (_this$getLocalTracks = this.getLocalTracks(mediaType)) === null || _this$getLocalTracks === void 0 ? void 0 : _this$getLocalTracks.length);
    newTrack.setSourceName(sourceName);
  }

  this.rtc.addLocalTrack(newTrack);
  newTrack.setConference(this); // Add event handlers.
...
JitsiConference.prototype._setNewVideoType = function (track) {
  if (FeatureFlags.isSourceNameSignalingEnabled() && track) {
    ...
    this._signalingLayer.setTrackVideoType(track.getSourceName(), track.videoType); // TODO: Optimize to detect whether presence was changed, for now always report changed to send presence


    return true;
  }

  const videoTypeTagName = 'videoType'; // if track is missing we revert to default type Camera, the case where we screenshare and
  // we return to be video muted

  const trackVideoType = track ? track.videoType : VideoType.CAMERA; // if video type is camera and there is no videoType in presence, we skip adding it, as this is the default one
