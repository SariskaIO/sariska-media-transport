...
export default function JitsiConference(options) {
  ...
  this.xmpp = (_this$connection = this.connection) === null || _this$connection === void 0 ? void 0 : _this$connection.xmpp;
  ...
  this.isVideoMutedByFocus = false; // when video muted by focus we receive the jid of the initiator of the mute

  this.mutedVideoByFocusActor = null; // Flag indicates if the 'onCallEnded' method was ever called on this
...
JitsiConference.prototype._init = function (options = {}) {
  ...
  const {
    config
  } = this.options; // Get the codec preference settings from config.js.
  ...
  this._signalingLayer.setChatRoom(this.room); // Connection interrupted/restored listeners
  ...
  this.participantConnectionStatus.init(); // Add the ability to enable callStats only on a percentage of users based on config.js settings.
  ...
  this.eventManager.setupStatisticsListeners(); // Disable VAD processing on Safari since it causes audio input to
  ...
  this.jvb121Status = new Jvb121EventGenerator(this); // creates dominant speaker detection that works only in p2p mode
  ...
  this.setLocalParticipantProperty('codecType', this.codecSelection.getPreferredCodec()); // Set transcription language presence extension.
...
JitsiConference.prototype.authenticateAndUpgradeRole = function (options) {
  return authenticateAndUpgradeRole.call(this, { ...options,
...
JitsiConference.prototype.setDisplayName = function (name) {
  if (this.room) {
    const nickKey = 'nick'; // if there is no display name already set, avoid setting an empty one
...
JitsiConference.prototype.getTranscriber = function () {
  if (this.transcriber === undefined) {
    this.transcriber = new Transcriber(); // add all existing local audio tracks to the transcriber
...
JitsiConference.prototype._fireMuteChangeEvent = function (track) {
  ...
  if (this.isMutedByFocus && track.isAudioTrack() && !track.isMuted()) {
    ...
    this.isVideoMutedByFocus = false; // unmute local user on server
...
JitsiConference.prototype.onLocalTrackRemoved = function (track) {
  ...
  track.removeEventListener(JitsiTrackEvents.TRACK_AUDIO_LEVEL_CHANGED, track.audioLevelHandler); // send event for stopping screen sharing
...
JitsiConference.prototype.replaceTrack = function (oldTrack, newTrack) {
  const oldTrackBelongsToConference = this === (oldTrack === null || oldTrack === void 0 ? void 0 : oldTrack.conference); // First do the removal of the oldTrack at the JitsiConference level
  ...
  return this._doReplaceTrack(oldTrackBelongsToConference ? oldTrack : null, newTrack).then(() => {
    ...
    newTrack && this._setupNewTrack(newTrack); // Send 'VideoTypeMessage' on the bridge channel when a video track is added/removed.
...
JitsiConference.prototype._setNewVideoType = function (track) {
  if (FeatureFlags.isSourceNameSignalingEnabled()) {
    ...
    this._signalingLayer.setTrackVideoType(getSourceNameForJitsiTrack(this.myUserId(), track.getType(), 0), track.videoType); // TODO: Optimize to detect whether presence was changed, for now always report changed to send presence


    return true;
  }

  const videoTypeTagName = 'videoType'; // if video type is camera and there is no videoType in presence, we skip adding it, as this is the default one
  ...
  if (track.videoType !== VideoType.CAMERA || this.room.getFromPresence(videoTypeTagName)) {
    ...
    // and the mute status
    return this.room.addOrReplaceInPresence(videoTypeTagName, {
...
JitsiConference.prototype._setTrackMuteStatus = function (localTrack, isMuted) {
  if (FeatureFlags.isSourceNameSignalingEnabled()) {
    ...
    this._signalingLayer.setTrackMuteStatus(getSourceNameForJitsiTrack(this.myUserId(), localTrack.getType(), 0), isMuted);
  }
...
JitsiConference.prototype._removeLocalTrackAsMute = function (track) {
  ...
  return Promise.allSettled(removeAsMutePromises).then(() => {
    // Signal the video type to the bridge.
...
JitsiConference.prototype.setLastN = function (lastN) {
  ...
  this.receiveVideoController.setLastN(n); // If the P2P session is not fully established yet, we wait until it gets
  // established.
...
JitsiConference.prototype.onMemberJoined = function (jid, nick, role, isHidden, statsID, status, identity, botType, fullJid, features, isReplaceParticipant) {
  ...
  this._updateFeatures(participant); // maybeStart only if we had finished joining as then we will have information for the number of participants
...
JitsiConference.prototype.onRemoteTrackAdded = function (track) {
  ...
  track.addEventListener(JitsiTrackEvents.TRACK_AUDIO_LEVEL_CHANGED, (audioLevel, tpc) => {
...
JitsiConference.prototype._acceptJvbIncomingCall = function (jingleSession, jingleOffer, now) {
  ...
  if (this.wasStopped) {
    Statistics.sendAnalyticsAndLog(createJingleEvent(ACTION_JINGLE_RESTART, {
      p2p: false
    }));
  }
  ...
  try {
    jingleSession.initialize(this.room, this.rtc, this._signalingLayer, { ...this.options.config,
...
JitsiConference.prototype.onCallEnded = function (jingleSession, reasonCondition, reasonText) {
  ...
  if (jingleSession === this.jvbJingleSession) {
    ...
    Statistics.sendAnalytics(createJingleEvent(ACTION_JINGLE_TERMINATE, {
      p2p: false
    })); // Stop the stats
    ...
    const stopOptions = {}; // It's the responder who decides to enforce JVB mode, so that both
...
JitsiConference.prototype.getSsrcByTrack = function (track) {
  var _this$getActivePeerCo;

  return track.isLocal() ? (_this$getActivePeerCo = this.getActivePeerConnection()) === null || _this$getActivePeerCo === void 0 ? void 0 : _this$getActivePeerCo.getLocalSSRC(track) : track.getSSRC();
};
/**
...
JitsiConference.prototype._acceptP2PIncomingCall = function (jingleSession, jingleOffer) {
  ...
  this.p2pJingleSession.initialize(this.room, this.rtc, this._signalingLayer, { ...this.options.config,
    enableInsertableStreams: this.isE2EEEnabled()
  });
...
JitsiConference.prototype._updateProperties = function (properties = {}) {
  ...
  if (changed) {
    this.eventEmitter.emit(JitsiConferenceEvents.PROPERTIES_CHANGED, this.properties); // Some of the properties need to be added to analytics events.
...
JitsiConference.prototype._setP2PStatus = function (newStatus) {
  ...
  this.eventEmitter.emit(JitsiConferenceEvents._MEDIA_SESSION_ACTIVE_CHANGED, this._getActiveMediaSession()); // Refresh connection interrupted/restored
...
JitsiConference.prototype._maybeStartOrStopP2P = function (userLeftEvent) {
  if (!this.isP2PEnabled() || this.isP2PTestModeEnabled() || browser.isFirefox() || browser.isWebKitBased() || this.isE2EEEnabled()) {
...
JitsiConference.prototype._stopP2PSession = function (options = {}) {
  ...
  this.p2pJingleSession.terminate(() => {
    ...
    reason,
    reasonDescription,
    sendSessionTerminate: this.room && this.getParticipantById(Strophe.getResourceFromJid(this.p2pJingleSession.remoteJid))
  });
...
JitsiConference.prototype._restartMediaSessions = function () {
  ...
  if (this.jvbJingleSession) {
    this.jvbJingleSession.terminate(null
    /* success callback => we don't care */
    , error => {
...
JitsiConference.prototype.enableAVModeration = function (mediaType) {
  if (this.room && this.isModerator() && (mediaType === MediaType.AUDIO || mediaType === MediaType.VIDEO)) {
...
JitsiConference.prototype.getBreakoutRooms = function () {
  ...
  return (_this$room = this.room) === null || _this$room === void 0 ? void 0 : _this$room.getBreakoutRooms();
};
/**
...
JitsiConference.prototype.handleSubtitles = function () {
  ...
  this.on(JitsiConferenceEvents.ENDPOINT_MESSAGE_RECEIVED, (participant, json) => {
    ...
    try {
      var _conference$participa, _conference$participa2, _conference$participa3, _json$participant, _conference, _conference$getLocalU, _json$participant2;
      ...
      const fellowParticipantName = ((_conference$participa = conference.participants[json === null || json === void 0 ? void 0 : (_json$participant = json.participant) === null || _json$participant === void 0 ? void 0 : _json$participant.id]) === null || _conference$participa === void 0 ? void 0 : (_conference$participa2 = _conference$participa._identity) === null || _conference$participa2 === void 0 ? void 0 : (_conference$participa3 = _conference$participa2.user) === null || _conference$participa3 === void 0 ? void 0 : _conference$participa3.name) || ((_conference = conference) === null || _conference === void 0 ? void 0 : (_conference$getLocalU = _conference.getLocalUser()) === null || _conference$getLocalU === void 0 ? void 0 : _conference$getLocalU.name);
