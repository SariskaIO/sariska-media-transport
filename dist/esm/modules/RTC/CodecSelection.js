...
export class CodecSelection {
    ...
    _selectPreferredCodec(mediaSession) {
        ...
        const currentCodec = session === null || session === void 0 ? void 0 : session.peerconnection.getConfiguredVideoCodec();
        let selectedCodec = preferredCodec !== null && preferredCodec !== void 0 ? preferredCodec : currentCodec;
        if (!this.enforcePreferredCodec) {
            const remoteParticipants = this.conference.getParticipants().map(participant => participant.getId());
            const remoteCodecs = remoteParticipants === null || remoteParticipants === void 0 ? void 0 : remoteParticipants.map(remote => {
                const peerMediaInfo = session._signalingLayer.getPeerMediaInfo(remote, MediaType.VIDEO);
                return peerMediaInfo === null || peerMediaInfo === void 0 ? void 0 : peerMediaInfo.codecType;
            });
            ...
            if (nonPreferredCodecs.length) {
                ...
                selectedCodec = (_a = nonPreferredCodecs.find(codec => codec === CodecMimeType.VP8)) !== null && _a !== void 0 ? _a : nonPreferredCodecs.find(codec => this._isCodecSupported(codec));
            }
        }
        if (selectedCodec !== currentCodec || !(session === null || session === void 0 ? void 0 : session.peerconnection.isVideoCodecDisabled(disabledCodec))) {
